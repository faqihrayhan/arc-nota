  ...
  return (
    <header className={cn("sticky top-0 z-50 transition-all duration-500", scrolled ? "border-b border-ink-line/60 bg-ink/85 backdrop-blur-2xl shadow-[0_1px_40px_-12px_rgba(0,0,0,0.4)]" : "border-b border-transparent bg-ink/50 backdrop-blur-md")}>
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3.5">
        {/* Logo */}
        <a href="#top" className="group flex items-center gap-2.5 transition-all duration-300">
        ...
        <nav className="hidden items-center gap-0.5 md:flex">
          {links.map((l) => {
            ...
            return (
              <a key={l.href} href={l.href} className={cn("relative px-4 py-2 text-sm rounded-xl transition-all duration-300 group", isActive ? "text-text bg-white/[0.06]" : "text-text-muted hover:text-text hover:bg-white/[0.03]")}>
          ...
          <div className="relative" ref={docsRef}>
            ...
            <div className={cn("absolute top-full left-1/2 -translate-x-1/2 mt-3 w-72 overflow-hidden rounded-2xl border border-ink-line/60 bg-ink-2/95 backdrop-blur-2xl shadow-2xl shadow-black/40 transition-all duration-300 origin-top", docsOpen ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 -translate-y-2 pointer-events-none")}>
              ...
              <div className="border-t border-ink-line/40 px-4 py-3">
                <a href="https://github.com" target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs text-text-muted hover:text-text transition-colors">
          ...
          {links.map((l) => {
            ...
            return (
              <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className={cn("flex items-center gap-3 px-4 py-3 text-sm rounded-xl transition-all duration-300", isActive ? "text-text bg-white/[0.06]" : "text-text-muted hover:text-text hover:bg-white/[0.03]")}>
                <span className={cn("w-1.5 h-1.5 rounded-full transition-all duration-300", isActive ? "bg-paper-white" : "bg-ink-line")} />
